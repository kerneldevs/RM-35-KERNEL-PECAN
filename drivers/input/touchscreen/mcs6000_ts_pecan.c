...
int mcs6000_ts_ioctl_down(struct inode *inode, struct file *flip, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case MCS6000_TS_DOWN_IOCTL_INTR_HIGH:
			gpio_direction_output(dev->intr_gpio, GPIOF_DRIVE_OUTPUT | GPIOF_OUTPUT_HIGH);
			break;
		case MCS6000_TS_DOWN_IOCTL_INTR_LOW:
			gpio_direction_output(dev->intr_gpio, GPIOF_DRIVE_OUTPUT | GPIOF_OUTPUT_LOW);
			break;
...
// LGE_CHANGE_S dangwoo.choi@lge.com - for MCS7000
		case MCS6000_TS_DOWN_IOCTL_SDA_INPUT :
			gpio_direction_output(dev->sda_gpio, GPIOF_INPUT);
			break;
